  ...
  test('loads and returns URI of remote key', async () => {
    const owner = new Person({
      account: new RemoteAccount({
        inbox: { uri: '' },
        publicKey: {
          uri: 'https://OwNeR.xn--kgbechtv/',
          publicKeyPem: ''
        },
        uri: ''
      }),
      username: '',
      host: 'FiNgEr.OwNeR.إختبار'
    });

    await repository.insertRemoteAccount(owner.account);
  ...
  test('loads account and returns true if signature is valid', async () => {
    const owner = new Person({
      account: new RemoteAccount({
        inbox: { uri: '' },
        publicKey: {
          uri: 'https://OwNeR.xn--kgbechtv/users/admin#main-key',
...
`
    ...
    await expect(key.verifySignature(repository, signature))
      .resolves
      .toBe(true);
  });

  test('loads account and returns false if signature is valid', async () => {
    const owner = new Person({
      account: new RemoteAccount({
        inbox: { uri: '' },
        publicKey: {
...
describe('selectPrivateKeyPem', () =>
  test('loads and returns private key in PEM', async () => {
    ...
    await expect(key.selectPrivateKeyPem(repository))
      .resolves
      .toBe(privateKeyPem);
  }));
...
describe('toActivityStreams', () =>
  test('returns ActivityStreams representation', async () => {
    ...
    await expect(key.toActivityStreams(repository)).resolves.toEqual({
...
describe('generate', () => test('returns a string', () =>
  expect(Key.generate()).toMatch(/^-----BEGIN RSA PRIVATE KEY-----\n(.|\n)*\n-----END RSA PRIVATE KEY-----\n$/)));
