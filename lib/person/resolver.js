...
export default {
  async resolveByUsernameAndNormalizedHost(repository, username, normalizedHost) {
    if (normalizedHost) {
      ...
      const host = firstFinger.object.subject.split('@', 2)[0];
      const { href } =
  ...
  async resolveByKeyUri(repository, uri) {
    ...
    const [, host] = firstFinger.subject.split('@', 2);

    return this.fromActivityStreams(repository, domainToASCII(host), object);
  }
};
