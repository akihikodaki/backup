...
const database = `${process.env.PGDATABASE || process.env.USER}_miniverse_test_${process.env.JEST_WORKER_ID || '1'}`;
const redisOptions = {
  keyPrefix: `miniverse_test_${process.env.JEST_WORKER_ID}`
};
...
afterEach(() => Promise.all([
  ...
  redis.client.keys(`${redisOptions.keyPrefix}*`).then(keys =>
    keys.length && redis.client.del(keys.map(key =>
      key.slice(redisOptions.keyPrefix.length))))
]));
