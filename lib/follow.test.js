...
describe('selectRemoteAccountByActor', () =>
  test('loads and returns actor remote account', async () => {
    ...
    expect(result).toHaveProperty(
      ['publicKey', 'uri'], 'https://AcToR.إختبار/#publickey');

    expect(result).toHaveProperty('uri', 'https://AcToR.إختبار/');
  }));
...
describe('toActivityStreams', () =>
  test('loads and returns ActivityStreams representation', async () => {
    ...
    await expect((await follow.toActivityStreams(repository)).body)
      .resolves
      .toEqual({
        type: 'Follow',
...
describe('fromActivityStreams', () => {
  test('creates follow from ActivityStreams representation', async () => {
    ...
    nock('https://AcToR.إختبار').post('/inbox').reply(200);
