/*
  Copyright (C) 2018  Akihiko Odaki <nekomanma@pixiv.co.jp>

  This program is free software: you can redistribute it and/or modify
  it under the terms of the GNU Affero General Public License as published by
  the Free Software Foundation, version 3 of the License.

  This program is distributed in the hope that it will be useful,
  but WITHOUT ANY WARRANTY; without even the implied warranty of
  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
  GNU Affero General Public License for more details.

  You should have received a copy of the GNU Affero General Public License
  along with this program.  If not, see <https://www.gnu.org/licenses/>.
*/

import ActivityStreams from './activitystreams';
import Follow, { RemoteObjectError } from './follow';
import LocalAccount from './local_account';
import Person from './person';
import RemoteAccount from './remote_account';
import repository from './test_repository';
const nock = require('nock');

const privateKeyPem = `-----BEGIN RSA PRIVATE KEY-----

OyJRYe+sFKZ6lXqnwdWuTrxTNucFuhw+6BVyzNn6lI5cNXLr1reH
-----END RSA PRIVATE KEY-----
`;

async function fabricateFollow(actor, object) {
  const follow = new Follow({ actor, object });
  await repository.insertFollow(follow);
  return follow;
}

async function fabricateLocalActor() {
  const actor = new Person({
    account: new LocalAccount({
      admin: true,
      privateKeyPem,
      salt: '',
      serverKey: '',
      storedKey: ''
    }),
    username: 'actor',
    host: null
  });

  await repository.insertLocalAccount(actor.account);

  return actor;
}

async function fabricateLocalObject() {
  const object = new Person({
    account: new LocalAccount({
      admin: true,
      privateKeyPem,
      salt: '',
      serverKey: '',
      storedKey: ''
    }),
    username: 'object',
    host: null
  });

  await repository.insertLocalAccount(object.account);

  return object;
}

async function fabricateRemoteActor() {
  const actor = new Person({
    account: new RemoteAccount({
      inbox: { uri: 'http://origin.actor.example.com/inbox' },
      publicKey: {
        uri: 'http://origin.actor.example.com/#publickey',
        publicKeyPem: 'public key PEM'
      },
      uri: 'http://origin.actor.example.com/'
    }),
    username: 'actor',
    host: 'host.actor.example.com'
  });

  await repository.insertRemoteAccount(actor.account);

  return actor;
}

describe('selectPersonByActor', () =>
  test('loads and returns actor person', async () => {
    const actor = await fabricateLocalActor();
    const object = await fabricateLocalObject();
    const follow = await fabricateFollow(actor, object);
    follow.actor = new Person({ id: actor.id });

    const result = await follow.selectPersonByActor(repository);

    expect(result).toHaveProperty('username', 'actor');
    expect(result).toHaveProperty('host', null);
  }));

describe('selectPersonByObject', () =>
  test('loads and returns object person', async () => {
    const actor = await fabricateLocalActor();
    const object = await fabricateLocalObject();
    const follow = await fabricateFollow(actor, object);
    follow.object = new Person({ id: object.id });

    const result = await follow.selectPersonByObject(repository);

    expect(result).toHaveProperty('username', 'object');
    expect(result).toHaveProperty('host', null);
  }));

describe('selectRemoteAccountByActor', () =>
  test('loads and returns actor remote account', async () => {
    const actor = await fabricateRemoteActor();
    const object = await fabricateLocalObject();
    const follow = await fabricateFollow(actor, object);
    follow.actor = new Person({ id: follow.actor.id });

    const result = await follow.selectRemoteAccountByActor(repository);

    expect(result).toHaveProperty(
      ['inbox', 'uri'], 'http://origin.actor.example.com/inbox');

    expect(result).toHaveProperty(
      ['publicKey', 'uri'], 'http://origin.actor.example.com/#publickey');

    expect(result).toHaveProperty('uri', 'http://origin.actor.example.com/');
  }));

describe('toActivityStreams', () =>
  test('loads and returns ActivityStreams representation', async () => {
    const actor = await fabricateLocalActor();
    const object = await fabricateLocalObject();
    const follow = await fabricateFollow(actor, object);
    follow.actor = new Person({ id: follow.actor.id });
    follow.object = new Person({ id: follow.object.id });

    await expect(follow.toActivityStreams(repository))
      .resolves
      .toHaveProperty('body', {
        type: 'Follow',
        actor: 'https://origin.example.com/@actor',
        object: 'https://origin.example.com/@object'
      });
  }));

describe('create', () => {
  test('creates follow', async () => {
    const actor = await fabricateLocalActor();
    const object = await fabricateLocalObject();

    const follow = await Follow.create(repository, actor, object);

    expect(follow).toBeInstanceOf(Follow);
    expect(follow).toHaveProperty('actor', actor);
    expect(follow).toHaveProperty('object', object);
    expect((await repository.selectLocalAccountsByFollowee(object))[0])
      .toBeInstanceOf(LocalAccount);
  });

  /*
    ActivityPub
    6.5 Follow Activity
    https://www.w3.org/TR/activitypub/#follow-activity-outbox
    > The side effect of receiving this in an outbox is that the server SHOULD
    > add the object to the actor's following Collection when and only if an
    > Accept activity is subsequently received with this Follow activity as its
    > object.
  */
  test('creates accept activity', async () => {
    const actor = await fabricateRemoteActor();
    const object = await fabricateLocalObject();

    const post = nock('http://origin.actor.example.com')
      .post('/inbox')
      .reply(200);

    try {
      await Follow.create(repository, actor, object);
      expect(post.isDone()).toBe(true);
    } finally {
      nock.cleanAll();
    }
  });
});

describe('fromActivityStreams', () => {
  test('create follow from ActivityStreams representation', async () => {
    const actor = await fabricateRemoteActor();
    const object = await fabricateLocalObject();
    const activity = new ActivityStreams({
      type: 'Follow',
      object: 'https://origin.example.com/@object'
    });

    const post = nock('http://origin.actor.example.com')
      .post('/inbox')
      .reply(200);

    try {
      const follow =
        await Follow.fromActivityStreams(repository, actor, activity);

      expect(follow).toHaveProperty('actor', actor);

      await expect(follow.selectPersonByObject(repository))
        .resolves
        .toHaveProperty('id', object.id);
    } finally {
      nock.cleanAll();
    }
  });

  test('rejects with an instance of RemoteObjectError if object is remote',
    async () => {
      const actor = await fabricateRemoteActor();
      const activity = new ActivityStreams({
        type: 'Follow',
        object: 'https://origin.remote.example.com/'
      });

      expect(Follow.fromActivityStreams(repository, actor, activity))
        .rejects
        .toBeInstanceOf(RemoteObjectError);
    });
});
