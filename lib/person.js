...
export default class {
  ...
  async toActivityStreams(repository) {
    ...
    const { privateKeyPem } = await repository.selectLocalAccountByPerson(this);
  ...
  static async fromActivityStreams(repository, host, object) {
    ...
    const publicKey = object.getPublicKey();
    ...
    const person = new this({
      account: new RemoteAccount({
        uri: id,
        inbox: { uri: object.body.inbox },
        publicKey: {
          uri: publicKeyId,
          publicKeyPem: publicKey.body.publicKeyPem
        }
      }),
      username: object.body.preferredUsername,
      host
    });
  ...
  static async resolveByKeyUri(repository, uri) {
    ...
    if (object.getPublicKey().getId() != uri) {
      throw new Error;
    }

    const { object: firstFinger } = await lookup(object.getId());
