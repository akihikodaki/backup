...
export default function Repository({ console, host, fingerHost, pg, redis }) {
  ...
  this.host = domainToUnicode(host);
  this.fingerHost = fingerHost ? domainToUnicode(fingerHost) : this.host;
  this.pg = pg;
  this.userAgent = `ActiveNode (${domainToASCII(host)})`;
  this.loadeds = new WeakSet;

  this.redis = {
    url: typeof redis[0] == 'string' ? redis[0] : null,
