...
`
    ...
    expect(account).toHaveProperty(['person', 'host'],
      'finger.remote.xn--kgbechtv');
    expect(account).toHaveProperty('uri',
      'https://remote.xn--kgbechtv/@name%20of%20user');
    expect(account).toHaveProperty(['publicKey', 'uri'],
      'https://remote.xn--kgbechtv/@name%20of%20user#key');
    expect(account).toHaveProperty(['publicKey', 'publicKeyPem'], `-----BEGIN RSA PUBLIC KEY-----

...
`);
    ...
    await expect(repository.selectRemoteAccountByUsernameAndNormalizedHost('name of user',  'finger.remote.xn--kgbechtv'))
      .resolves
      .toHaveProperty('uri', 'https://remote.xn--kgbechtv/@name%20of%20user');
  });
