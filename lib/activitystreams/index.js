...
function getChild(body) {
  return new this.constructor(body, this.normalizedHost, resolvers.get(this));
}
...
export default class {
  ...
  async act(repository, actor) {
    const [type] = await Promise.all([
      this.getType(repository),
      Promise.all([
        this.getActor(repository).then(actual => actual.getId()),
  ...
  async create(repository, attributedTo) {
    const [type] = await Promise.all([
      this.getType(repository),
      Promise.all([
        this.getAttributedTo(repository).then(actual => actual.getId()),
