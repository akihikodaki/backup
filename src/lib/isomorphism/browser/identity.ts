...
/*
  ...
  > use.
*/
function hash(string: ArrayBuffer) {
...
*/
function hmac(key: CryptoKey, string: Uint8Array) {
...
*/
async function iterateHash(string: Uint8Array, salt: ArrayBuffer, iterations: number) {
...
*/
function xor(buffer0: ArrayBuffer, buffer1: ArrayBuffer) {
...
export async function signIn(user: Readonly<LocalActor>, password: string, nonce: string) {
  ...
  const serverNonceUri = 'data:;base64,' + nonce;
...
export async function signUp(username: string, password: string) {
  ...
  const [serverKey, storedKey] = await Promise.all([
    hmac(saltedPassword, encoder.encode('Server Key')),
    hmac(saltedPassword, encoder.encode('Client Key')).then(hash)
  ]);
