...
export default {
  create(server) {
    ...
    webSocketServer.on('connection', (connection, { user }) => {
      this.selectRecentNotesFromInbox(user).then(orderedItems => {
        const collection = new OrderedCollection({ orderedItems });
        const activityStreams = collection.toActivityStreams(this);

        activityStreams['@context'] = 'https://www.w3.org/ns/activitystreams';
        connection.send(JSON.stringify(activityStreams));
      });
    });
  }
}
